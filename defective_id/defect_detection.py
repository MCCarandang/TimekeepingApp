...
class AccessGrantedWindow(QMainWindow):
    ...
    def is_valid_user(self, rfid_tag):
        conn = sqlite3.connect('/home/raspberrypi/Desktop/TimekeepingApp/timekeepingapp.db')
        cursor = conn.cursor()
        cursor.execute("SELECT COUNT(*) FROM employees WHERE rfid = ?", (rfid_tag,))
        result = cursor.fetchone()
        conn.close()
        return result[0] > 0

    def handle_denied_user(self, rfid_tag):
        self.message_label.setText("ACCESS DENIED")
        self.message_label.setFont(QFont("Helvetica", 35, QFont.Bold))
        self.message_label.setStyleSheet("color: red;")
        photo_path = self.capture_denied_photo(self.current_state[0])  # 'I' or 'O'

        # Log into denied users table
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        conn = sqlite3.connect('/home/raspberrypi/Desktop/TimekeepingApp/timekeepingapp.db')
        cursor = conn.cursor()
        cursor.execute("INSERT INTO denied_usr (rfid, timestamp, photo_path) VALUES (?, ?, ?)", (rfid_tag, timestamp, photo_path))
        conn.commit()
        conn.close()

    def handle_defective_id(self):
        current_time = time.strftime("%Y-%m-%d %H:%M:%S")

        # Show UI warning
        self.message_label.setText("DEFECTIVE ID DETECTED")
        self.message_label.setFont(QFont("Helvetica", 20, QFont.Bold))
        self.message_label.setStyleSheet("color: white;")
        self.transaction_code_label.setText("")
        ...
        try:
            ...
            filename = f"defective_{time.strftime('%Y%m%d_%H%M%S')}.jpg"
            file_path = os.path.join(save_dir, filename)
            self.camera.capture(file_path)

            # Display photo temporarily
            ...
            pixmap = QPixmap.fromImage(qimage)
            self.camera_label.setPixmap(pixmap)
            QTimer.singleShot(2000, self.clear_camera_label)
            ...
            conn = sqlite3.connect('/home/raspberrypi/Desktop/TimekeepingApp/timekeepingapp.db')
            cursor = conn.cursor()
            cursor.execute("""INSERT INTO def_ids (photo, timestamp) VALUES (?, ?)""", (photo_blob, current_time))
            conn.commit()
            conn.close()
        except Exception as e:
            print(f"Defective ID capture failed: {e}")
            ...
            try:
                ...
                if not id:
                    ...
                    if self.fail_start_time is None:
                        self.fail_start_time = now
                        self.failed_reads = 1
                    else:
                        if now - self.fail_start_time < 10:  # 10 seconds window
                            self.failed_reads += 1
                        else:
                            self.fail_start_time = now
                            self.failed_reads = 1

                    if self.failed_reads >= 3:  # 3 consecutive failures within 10 seconds
                        self.handle_defective_id()
                        self.failed_reads = 0
                        self.fail_start_time = None
                    return  # Stop further processing until a tag is detected

                # Reset failed read tracking on successful read
                self.failed_reads = 0
                self.fail_start_time = None

                rfid_tag = str(id)

                if rfid_tag in SPECIAL_RFID_TAGS:
                    self.handle_special_tag()
                elif self.is_valid_user(rfid_tag):
                    self.handle_rfid_scan(rfid_tag)
                else:
                    self.handle_denied_user(rfid_tag)
